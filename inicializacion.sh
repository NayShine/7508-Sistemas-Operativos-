...
leer_config () 
{
	...
	done < "$CONF/config.txt" 

	log "leer_config" "INF" "Se leyo el archivo de configuracion para obtener los directorios definidos"
	...
	then
		...
		log "leer_config" "INF" "Se detuvo la inicializacion ya que faltan directorios"
	fi
}
...
dar_permisos ()
{
	...
	if [ -d "$BINARIOS" ]
	then
		find "$BINARIOS" -type f -exec chmod +rx {} +
		echo "Se dieron permisos de lectura y ejecucion al directorio que contiene los archivos ejecutables"
		log "dar_permisos" "INF" "Se dieron permisos de lectura y ejecucion al directorio $BINARIOS"
		echo
	else
		...
		log "dar_permisos" "INF" "No se pudieron dar permisos de lectura y ejecucion al directorio $BINARIOS"
		echo
	fi
}
